...
PNG grayscale(PNG image) {
  ...
  for (unsigned x = 0; x < image.width(); x++) {
    for (unsigned y = 0; y < image.height(); y++) {
      ...
      double grayvalue = 0; // this will be changed once you determine what the adjusted RGB values will be below!
  ...
  for (unsigned x = 0; x < image.width(); x++) {
    for (unsigned y = 0; y < image.height(); y++) {
      ...
      if(sqrt((x-centerX)*(x-centerX)+(y-centerY)*(y-centerY))>=200){
        ...
        pixel->r = (1-0.005*(sqrt((x-centerX)*(x-centerX)+(y-centerY)*(y-centerY)))) * pixel->r;
      pixel->g = (1-0.005*(sqrt((x-centerX)*(x-centerX)+(y-centerY)*(y-centerY)))) * pixel->g;
      pixel->b = (1-0.005*(sqrt((x-centerX)*(x-centerX)+(y-centerY)*(y-centerY)))) * pixel->b;
  ...
  for (unsigned x = 0; x < secondImage.width(); x++) {
    for (unsigned y = 0; y < secondImage.height(); y++) {
         RGBAPixel* pixel1 = firstImage.getPixel(x, y);
          ...
          unsigned char m = (unsigned char)255;
          if(pixel2->r ==m && pixel2->g == m && pixel2->b ==m){

            if(pixel1->r >(unsigned char)215){
              pixel1->r =(unsigned char)255;

            }else{
              pixel1->r +=(unsigned char)40;
            }
            
            
            if(pixel1->g >(unsigned char)215){
              pixel1->g =(unsigned char)255;

            }else{
              pixel1->g +=(unsigned char)40;

            }
            
            if(pixel1->b >(unsigned char)215){
              pixel1->b =(unsigned char)255;

            }else{
              pixel1->b += (unsigned char)40;
